...
### Values and Types
Like most scripting languages, Kassite is dynamically typed. 
...
`int`: 32/64 bit signed integers.Number types are 64 bits by default, but you can define `Kassite_USE_32_BIT_NUMBERS` to use 32 bit numbers.

`float`: 32/64 bit signed floating point.
...
`string`: A mutable UTF8 character string.

`array`: A collection of other values. An array must be homogeneous (all values of the same type) and cannot contain other collections. Array data is acessed by integer index starting from 0.

`list`: A collection of other values. A list can be heterogeneous and contain any type. List data is acessed by integer index starting from 0. Slower than an `array` but more flexible.

`dictionary`: A collection of other values, but this time as a hashmap. A dictionary can contain any type like a list, but it can be indexed with any non-collection type.

`class`: A class type. Contains member variables and functions. Class functions are implicitly called from "class scope" so that means that to each of the class's functions, the class's variables are local.

### Type Coercion
There is minimal type coercion in Kassite. Int will be promoted to Float for any operation between the two types, and String will be promoted to a List of characters when operating with a List. Attempting to compare a number type to a string or a list (unless it's only 1 element long and that element is a number) will throw an error. If you want to purposely convert values, there are built in casting/parsing functions `bool()`, `int()`, `float()`, `string()`, `array()`, `list()`.
...
### Memory
Kassite is not technically garbage collected but does offer automatic memory management. It uses reference counting and scoping to free memory immediately after a value isn't being used anymore.
...
## Collections
A collection literal looks like `[value(s)...]` and a collection access looks like `name[index]`. A collection literal will resolve to an `array` if all the types are the same and a `list` otherwise.
