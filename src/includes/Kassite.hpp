...
#endif // KASSITE_USE_32_BIT_TYPES
...
#pragma warning( pop )
    ...
    // Kassite uses shared_ptr for ref counting, anything with a name
	...
	// KSLambda is a "native function" it's how you wrap c++ code for use inside Kassite
	...
	struct KSFunction {
		...
		// when using a Kassite function body
	...
	struct KSExpression {
        ...
        bool needsToReturn(KSExpressionRef expr, KSValueRef& returnVal, KassiteInterpreter* i) const;

        // walk the tree depth first and replace any function expressions 
		// with a value expression of their results
        KSExpressionRef consolidated(KassiteInterpreter* i) const;
	};
	...
	void KassiteInterpreter::newScope(const string& name) {
    ...
    KSScopeRef KassiteInterpreter::resolveScope(const string& name) {
        // if the scope exists we just use it as is
	...
	void KassiteInterpreter::closeCurrentScope() {
	...
	KSValueRef KassiteInterpreter::callFunction(const string& name, const KSList& args) {
	...
	KSValueRef KassiteInterpreter::callFunction(const KSFunctionRef fnc, const KSList& args) {
	...
	KSFunctionRef& KassiteInterpreter::newFunction(
        const string& name,
	...
	KSFunctionRef& KassiteInterpreter::newFunction(const string& name, const KSLambda& lam) {
	...
	KSFunctionRef& KassiteInterpreter::newLibraryFunction(
        const string& name,
        const KSLambda& lam,
        KSScopeRef scope) {
		auto oldScope = currentScope;
	...
	KSValueRef& KassiteInterpreter::resolveVariable(const string& name, KSScopeRef scope) {
        if (currentClass) {
	...
	KSFunctionRef KassiteInterpreter::resolveFunction(const string& name) {
		auto scope = currentScope;
		while (scope) {
	...
	KSExpressionRef KassiteInterpreter::getExpression(const vector<string>& strings) {
    ...
    bool KSExpression::needsToReturn(KSExpressionRef exp, KSValueRef& returnVal, KassiteInterpreter* i) const {
    ...
    KSExpressionRef KSExpression::consolidated(KassiteInterpreter* i) const {
        switch (type) {
	...
	KSValueRef KassiteInterpreter::getValue(const vector<string>& strings) {
		return get<KSValueRef>(getExpression(strings)->consolidated(this)->expression);
	}

	// evaluate an expression from expressionRef
	KSValueRef KassiteInterpreter::getValue(KSExpressionRef exp) {
	...
	void KassiteInterpreter::clearParseStacks() {
	...
	void KassiteInterpreter::closeDanglingIfExpression() {
	...
	bool KassiteInterpreter::closeCurrentExpression() {
	...
	void KassiteInterpreter::parse(const string& token) {
	...
	bool KassiteInterpreter::readLine(const string& text) {
        ++currentLine;
	...
	bool KassiteInterpreter::evaluate(const string& script) {
	...
	void KassiteInterpreter::clearState() {
		clearParseStacks();
	...
	KassiteInterpreter::KassiteInterpreter() {
